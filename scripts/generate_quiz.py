...
def main() -> int:
    ...
    parser.add_argument(
        "--quiz-kind",
        default="process",
        choices=["daily", "process", "checkpoint", "custom"],
        help="测验类型，用于匹配协议中的默认题量",
    )
    ...
    if question_count is None:
        if args.quiz_kind == "daily":
            question_count = protocol.get("daily_quiz_count") or protocol.get("process_quiz_count") or 4
        elif args.quiz_kind == "checkpoint":
            question_count = protocol.get("checkpoint_quiz_count") or 8
        else:
    ...
    bundle = {
        ...
        "protocol_defaults": {
            "daily_quiz_count": protocol.get("daily_quiz_count"),
            "process_quiz_count": protocol.get("process_quiz_count"),
            "checkpoint_quiz_count": protocol.get("checkpoint_quiz_count"),
    ...
    bank_row = (
        ...
        f"{table_cell('；'.join(focuses))} | {len(questions)} | {table_cell(bundle['quiz_kind_label'] + ' / ' + source_strategy)} | "
    ...
    print()
    print(f"测验类型：{bundle['quiz_kind_label']}")
