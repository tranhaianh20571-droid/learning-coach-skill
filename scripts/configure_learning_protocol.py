...
def protocol_questions(args: argparse.Namespace) -> list[str]:
    questions: list[str] = []
    if not args.target_days and not args.daily_minutes:
        questions.append("这轮准备几天完成、每天大概能投入多久？")
    ...
    if not args.diagnostic_mode:
        questions.append("默认先讲后测，还是允许在讲解前做诊断题？")
    if not args.daily_quiz_rule:
        questions.append("每天学习结束后是否固定做小测？如果做，按什么规则触发？")
    if args.daily_quiz_count is None:
        questions.append("每日小测每次几题更合适？")
    ...
    if not args.review_cadence:
        questions.append("复习节奏希望怎么安排？例如 1 天 / 3 天 / 7 天。")
...
def main() -> int:
    ...
    parser.add_argument("--target-days", default="", help="这轮计划几天完成")
    parser.add_argument("--daily-minutes", default="", help="每天可投入时间，例如 90 分钟")
    parser.add_argument(
        "--proactive-guidance",
        default="on",
    ...
    parser.add_argument("--diagnostic-mode", default="", help="默认先讲后测还是先诊断")
    parser.add_argument("--daily-quiz-rule", default="", help="每日小测触发规则")
    parser.add_argument("--daily-quiz-count", type=int, default=None, help="每日小测题量")
    ...
    parser.add_argument("--review-cadence", default="", help="复习节奏")
    ...
    protocol = {
        ...
        "target_days": args.target_days or "待确认",
        "daily_minutes": args.daily_minutes or "待确认",
        ...
        "daily_quiz_rule": args.daily_quiz_rule or "待确认",
        "daily_quiz_count": args.daily_quiz_count,
        ...
        "review_cadence": args.review_cadence or "待确认",
...
- 计划几天完成：{args.target_days or "待确认"}
- 每天可投入时间：{args.daily_minutes or "待确认"}

## 内容切块规则

- 什么算一块内容：{args.chunk_definition or "待确认"}
- 每块默认规模：{args.chunk_size or "待确认"}
- 默认先讲后测还是先诊断：{args.diagnostic_mode or "待确认"}

## 测验规则

- 每日小测触发：{args.daily_quiz_rule or "待确认"}
- 每日小测题量：{args.daily_quiz_count if args.daily_quiz_count is not None else "待确认"}
...
- 复习节奏：{args.review_cadence or "待确认"}
