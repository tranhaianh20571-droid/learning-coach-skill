...
def main() -> int:
    ...
    parser.add_argument(
        "--quiz-kind",
        default="process",
        choices=["daily", "process", "checkpoint"],
        help="测验类型",
    )
    parser.add_argument(
        "--quiz-result",
    ...
    if args.quiz_topic:
        ...
        kind_label = {"daily": "每日小测", "process": "过程测验", "checkpoint": "阶段测试"}[args.quiz_kind]
        progress_entry.append(f"- 测验：{kind_label} / {args.quiz_topic}（{result}）")
    if args.next_step:
    ...
    if args.quiz_topic:
        ...
        kind_label = {"daily": "每日小测", "process": "过程测验", "checkpoint": "阶段测试"}[args.quiz_kind]
        quiz_line = (
            f"| {table_cell(date.today().isoformat())} | {table_cell(args.quiz_topic)} | {table_cell(kind_label)} | "
