...
INTAKE_SCRIPT = SCRIPT_DIR / "configure_learning_intake.py"
...
DAILY_SCHEDULE_SCRIPT = SCRIPT_DIR / "generate_daily_schedule.py"
...
def read_context(root: Path) -> dict:
    return load_json(planning_dir(root) / "learning_context.json")


def context_ready(root: Path) -> bool:
    context = read_context(root)
    return bool(context) and not context.get("missing_questions")
...
def study_plan_ready(root: Path) -> bool:
    ...
    for row in rows:
        if (
            row.get("资料", "").strip()
            or row.get("产出物", "").strip()
            or row.get("目标日期", "").strip()
            or row.get("核心材料", "").strip()
            or row.get("证据 / 检查点", "").strip()
            or row.get("预计天数", "").strip()
            or row.get("对应证据", "").strip()
        ):
            return True
        goal = row.get("目标", "").strip() or row.get("目标结果", "").strip()
        if goal:
            return True
    return False
...
def build_study_plan(root: Path, topic: str) -> Path:
    ...
    protocol = read_protocol(root)
    context = read_context(root)
    active_materials = unique([
        row.get("材料", "").strip()
        for row in coverage["rows"]
        if row.get("材料", "").strip() and row.get("层级", "") != "本轮不展开"
    ])
    materials = active_materials or context.get("materials", []) or extract_bullets(session.get("当前资料", ""))
    materials_text = "；".join(materials) if materials else "待补充"
    focus_items = extract_bullets(session.get("当前重点", "")) or context.get("priorities", [])
    focus_text = "；".join(focus_items) if focus_items else "待确认重点"
    goal_text = compact_markdown_value(session.get("当前目标", "")) if session else context.get("final_goal", "待确认")
    target_days = context.get("target_days") or "待确认"
    daily_minutes = context.get("daily_minutes") or "待确认"
    daily_quiz_count = protocol.get("daily_quiz_count") or protocol.get("process_quiz_count") or "待确认"
    process_every = protocol.get("process_quiz_every", "待确认")

    if isinstance(target_days, int) and target_days > 0:
        phase_days = [
            max(1, round(target_days * 0.2)),
            max(1, round(target_days * 0.45)),
            max(1, round(target_days * 0.2)),
            max(1, target_days - (max(1, round(target_days * 0.2)) + max(1, round(target_days * 0.45)) + max(1, round(target_days * 0.2)))),
        ]
    else:
        phase_days = ["待确认", "待确认", "待确认", "待确认"]

    rows = [
        ("1", "建立范围、主线和语言映射", materials_text, "能说清当前目标、范围、主线结构与英文术语地图", phase_days[0], "进行中"),
        ("2", "吃透核心必讲与高频解释任务", materials_text, f"完成核心笔记、过程测验，并能解释“{focus_text}”", phase_days[1], "未开始"),
        ("3", "做迁移、变式和英文题面训练", materials_text, "能在英文题面下完成判断、解释、简答和变式题", phase_days[2], "未开始"),
        ("4", "复盘、查漏和模拟收束", materials_text, "完成阶段测试、错题回收与考前速记框架", phase_days[3], "未开始"),
    ]
    ...
    lines = [
        ...
        "## 设计依据",
        "",
        "- 先明确目标结果，再设计证据、材料和每天任务。",
        "- 资料中的点默认都要有去处，但不要求平均用力。",
        "- 每日任务默认包含：主线学习、主动回忆、必要复盘、结束小测。",
        "- 默认先讲后测；如果用户明确要求诊断或已有基础，再允许先做摸底题。",
        "",
        "## 目标与证据对齐",
        "",
        "| 目标结果 | 对应证据 | 主要材料 | 备注 |",
        "| --- | --- | --- | --- |",
        f"| 建立主线理解 | 能用自己的话解释当前主题、范围和核心术语 | {table_cell(materials_text)} | 避免只记零散点 |",
        f"| 提升 problem solving | 过程测验、每日小测、阶段测试 | {table_cell(materials_text)} | 重点检验迁移和解释 |",
        f"| 适应英文题面 | 看到英文关键词能定位模型和任务类型 | {table_cell(materials_text)} | 默认保留英文术语 |",
        "",
        "## 阶段安排",
        "",
    ...
    for stage, goal, materials_value, evidence, days, status in rows:
        lines.append(
            f"| {stage} | {table_cell(goal)} | {table_cell(materials_value)} | "
            f"{table_cell(evidence)} | {table_cell(str(days))} | {table_cell(status)} |"
        )
    ...
    lines.extend([
        ...
        "",
        f"- 计划总天数：{target_days}",
        f"- 每天可投入时间：{daily_minutes}",
        f"- 每日结束小测：默认 {daily_quiz_count} 题",
        f"- 过程测验规则：{process_every}",
        "- 具体每日任务请看 `daily_schedule.md`",
        "",
        "## 当前聚焦点",
        "",
        "- 当前阶段：阶段 1",
        f"- 当前单元：{goal_text if goal_text != '待确认' else '先完成资料分层和主线梳理'}",
        f"- 下一个检查点：完成当前块后，先做每日小测，再按 {process_every} 判断是否进入过程测验",
        "",
    ])

    path.write_text("\n".join(lines), encoding="utf-8")
    try:
        run_script(DAILY_SCHEDULE_SCRIPT, ["--root", str(root), "--topic", topic])
    except subprocess.CalledProcessError:
        print("[提示] 当前还无法生成 daily_schedule.md，请先补齐覆盖矩阵或初始化信息。")
    print(f"[写入] {path}")
    return path
...
def recommendation(root: Path) -> dict[str, str]:
    ...
    if not context:
        return {
            "key": "intake",
            "message": "先完成第一次完整初始化，把目标、时间预算、基础、颗粒度、材料和非目标一次性对齐。",
        }

    if context.get("missing_questions"):
        return {
            "key": "intake",
            "message": "完整初始化还有未确认项，建议先补齐再继续。",
        }
...
def show_status(root: Path) -> None:
    ...
    print("当前学习状态")
    print(f"- 根目录：{root}")
    print(f"- 初始化：{'已完成' if context_ready(root) else '待补齐'}")
    print(f"- 最终目标：{context.get('final_goal', '待确认')}")
    print(f"- 计划天数：{context.get('target_days', '待确认')}")
    print(f"- 每日时间：{context.get('daily_minutes', '待确认')}")
    print(f"- 默认颗粒度：{context.get('granularity_default', '待确认')}")
    print(f"- 默认先讲后测：{context.get('diagnostic_mode', '待确认')}")
    print(f"- 当前主题：{protocol.get('topic') or '待确认'}")
    print(f"- 主动带学：{protocol.get('proactive_guidance', '待确认')}")
    print(f"- 每日小测：{protocol.get('daily_quiz_rule', '待确认')} / {protocol.get('daily_quiz_count', '待确认')} 题")
...
def run_intake_wizard(root: Path, topic: str) -> None:
    context = read_context(root)
    print("先做一轮完整初始化，尽量把稳定信息一次问全，减少后续重复追问。")
    final_goal = ask_text("最终目标是什么", context.get("final_goal", "") or "学懂当前主题并通过目标测试")
    baseline_level = ask_choice(
        "你现在大概处在哪一层",
        [
            ("几乎没学过", "几乎没学过，需要从主线搭起"),
            ("学过但比较散", "学过一轮，但结构不稳"),
            ("会一些题但不系统", "能做部分题，但解释和迁移不稳"),
            ("基础还可以", "基础尚可，主要想冲刺提分"),
        ],
        context.get("baseline_level", "") or "学过但比较散",
    )
    current_round_note = ask_text("是否已经学过一轮，或目前最大的状态特点", context.get("current_round_note", "") or "学过一轮，但需要重新整合")
    english_readiness = ask_text("英文材料或英文试卷适应度", context.get("english_readiness", "") or "能看懂基本术语，但读题速度和表达还需提升")
    preferred_language = ask_text("偏好的讲解语言", context.get("preferred_language", "") or "中文主讲 + 核心术语保留英文")
    granularity_default = ask_choice(
        "默认讲解颗粒度",
        [("粗粒度", "先抓主线"), ("中粒度", "正常细讲"), ("细粒度", "逐点深挖")],
        context.get("granularity_default", "") or "中粒度",
    )
    diagnostic_mode = ask_choice(
        "默认先讲后测还是先诊断",
        [("先讲后测", "先讲清再做题"), ("可做轻量诊断", "允许先做少量摸底题"), ("先诊断后讲", "先测再决定从哪讲")],
        context.get("diagnostic_mode", "") or "先讲后测",
    )
    target_date = ask_text("目标日期（可留空）", context.get("target_date", "") or "")
    target_days_default = str(context.get("target_days", "") or "7")
    target_days = ask_int("计划几天完成这一轮", int(target_days_default) if target_days_default.isdigit() else 7)
    daily_minutes_default = str(context.get("daily_minutes", "") or "90")
    daily_minutes = ask_int("每天大概能投入多少分钟", int(daily_minutes_default) if daily_minutes_default.isdigit() else 90)
    weekly_days_default = str(context.get("study_days_per_week", "") or "7")
    study_days_per_week = ask_int("每周可学习几天", int(weekly_days_default) if weekly_days_default.isdigit() else 7)
    daily_completion_standard = ask_text(
        "你希望每天完成到什么程度才算达标",
        context.get("daily_completion_standard", "") or "当天主线能复述，能做完小测并知道错在哪里",
    )
    materials = ask_list("主要资料有哪些", context.get("materials", []))
    priorities = ask_list("当前最看重的重点", context.get("priorities", []) or ["理解主线", "Problem solving", "英文题面识别"])
    weaknesses = ask_list("已知薄弱点（可留空）", context.get("weaknesses", []))
    constraints = ask_list("限制条件（可留空）", context.get("constraints", []))
    non_goals = ask_list("当前非目标（可留空）", context.get("non_goals", []))
    note = ask_text("备注（可留空）", context.get("note", "") or "")
    ...
    intake_args = [
        ...
        "--daily-minutes", str(daily_minutes),
        "--study-days-per-week", str(study_days_per_week),
        "--baseline-level", baseline_level,
        "--current-round-note", current_round_note,
        "--english-readiness", english_readiness,
        "--preferred-language", preferred_language,
        "--granularity-default", granularity_default,
        "--diagnostic-mode", diagnostic_mode,
        "--daily-completion-standard", daily_completion_standard,
        "--note", note,
    ]
    for item in constraints:
        intake_args.extend(["--constraint", item])
    for item in priorities:
        intake_args.extend(["--priority", item])
    for item in weaknesses:
        intake_args.extend(["--weakness", item])
    for item in materials:
        intake_args.extend(["--material", item])
    for item in non_goals:
        intake_args.extend(["--non-goal", item])
    run_script(INTAKE_SCRIPT, intake_args)
...
def run_protocol_wizard(root: Path, topic: str, existing: dict | None = None) -> None:
    ...
    diagnostic_mode = ask_choice(
        "默认先讲后测还是先诊断？",
        [("先讲后测", "先讲清再做题"), ("可做轻量诊断", "允许先做少量摸底题"), ("先诊断后讲", "先测再决定从哪讲")],
        existing.get("diagnostic_mode", context.get("diagnostic_mode", "先讲后测")),
    )
    target_days = ask_text(
        "这轮准备几天完成",
        str(existing.get("target_days", context.get("target_days", "待确认"))),
    )
    daily_minutes = ask_text(
        "每天大概能投入多少时间",
        str(existing.get("daily_minutes", context.get("daily_minutes", "待确认"))),
    )
    daily_quiz_rule = ask_text(
        "每天结束后的小测怎么安排",
        existing.get("daily_quiz_rule", "每个学习日结束后做一次小测"),
    )
    daily_quiz_count = ask_int("每日小测每次几题", int(existing.get("daily_quiz_count") or 3))
    ...
    review_cadence = ask_text(
        "复习节奏怎么安排",
        existing.get("review_cadence", "1 天后 / 3 天后 / 7 天后"),
    )
    ...
    protocol_args = [
        ...
        "--diagnostic-mode", diagnostic_mode,
        "--daily-quiz-rule", daily_quiz_rule,
        "--daily-quiz-count", str(daily_quiz_count),
...
def run_align_wizard(root: Path, topic: str, args: argparse.Namespace | None = None) -> None:
    ...
    granularity = ask_choice(
        ...
        getattr(args, "granularity", "") or compact_markdown_value(session.get("讲解颗粒度", "")) or context.get("granularity_default", "中粒度") or "中粒度",
    )
    materials = ask_list("这轮使用哪些材料", getattr(args, "material", []) or extract_bullets(session.get("当前资料", "")) or context.get("materials", []))
    focuses = ask_list("这轮最重要的重点", getattr(args, "focus", []) or extract_bullets(session.get("当前重点", "")) or context.get("priorities", []))
    non_goals = ask_list("这轮先不展开哪些内容", getattr(args, "non_goal", []) or extract_bullets(session.get("当前非目标", "")) or context.get("non_goals", []))
...
- 每日小测规则：{protocol.get("daily_quiz_rule", "待确认")}
- 每日小测题量：{protocol.get("daily_quiz_count", "待确认")}
- 这一块结束后{"建议立即做过程测验" if quiz_due else "暂时不需要立即测验"}
- 默认授课策略：{context.get("diagnostic_mode", "待确认")}
...
def run_quiz_wizard(root: Path, topic: str, args: argparse.Namespace | None = None) -> None:
    session = read_session_sections(root)
    args = args or argparse.Namespace()
    quiz_kind = ask_choice(
        "这轮要生成哪种测验",
        [("daily", "每日小测"), ("process", "过程测验"), ("checkpoint", "阶段测试"), ("custom", "自定义")],
        getattr(args, "quiz_kind", "") or "process",
    )
...
def run_record_wizard(root: Path, topic: str, args: argparse.Namespace | None = None) -> None:
    ...
    if did_quiz:
        quiz_kind = ask_choice(
            "测验类型",
            [("daily", "每日小测"), ("process", "过程测验"), ("checkpoint", "阶段测试")],
            getattr(args, "quiz_kind", "") or "process",
        )
    ...
    if did_quiz:
        record_args.extend(["--quiz-topic", quiz_topic, "--quiz-kind", quiz_kind, "--quiz-result", quiz_result])
...
def run_guide(root_hint: str = "", topic_hint: str = "") -> int:
    ...
    context = read_context(root)
    if not context or context.get("missing_questions"):
        run_intake_wizard(root, topic)
    protocol = read_protocol(root)
    ...
    while True:
        ...
        action = ask_choice(
            ...
            rec["key"] if rec["key"] in {"intake", "coverage", "plan", "team", "block", "record", "quiz", "review", "align", "protocol", "status"} else "status",
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    intake = subparsers.add_parser("intake", help="补完整初始化信息")
    intake.add_argument("--root", default="", help="项目或笔记根目录")
    intake.add_argument("--topic", default="", help="当前学习主题")
    ...
    quiz.add_argument("--quiz-kind", default="", help="daily / process / checkpoint / custom")
    ...
    record.add_argument("--quiz-kind", default="", help="daily / process / checkpoint")
